...
function init_cocas_node_data(filepath)
    CSV.write(filepath,
        ...
        header = vec(hcat(:p, :o, :v, :a, :t, :w, :op, :sh, :σ))
    )
end
...
function init_cocas_edge_data(filepath)
    df = DataFrame([false 1 "None" 1 "day" "sunny" 1 1 1])
    push!(df, [true 1 "None" 1 "day" "sunny" 1 1 1])
    CSV.write(filepath, df;
        header = vec(hcat(:o, :l, :r, :a, :t, :w, :op, :sh, :σ))
    )
end

function init_cas_edge_data(filepath)
    df = DataFrame([false 1 "None" 1 "day" "sunny" 1])
    push!(df, [true 1 "None" 1 "day" "sunny" 1])
    CSV.write(filepath, df;
