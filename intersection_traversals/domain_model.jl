...
traffics = [:empty, :low, :medium, :high]
        ...
        for occ in 0:1
            for ped in 0:1
                for row in 0:1
                    for fol in 0:1
                        for tra in traffics
                            push!(S, State(pos, occ, ped, row, fol, tra))
                            if pos == :after
                                push!(G, State(pos, occ, ped, row, fol, tra))
                            end
                        end
                    end
                end
...
function generate_init(p, o, r, f, v)
    if v == 0
        return State(:approaching, o, p, r, f, :empty)
    elseif v < 3
        return State(:approaching, o, p, r, f, :low)
    elseif v < 5
        return State(:approaching, o, p, r, f, :medium)
    else
        return State(:approaching, o, p, r, f, :high)
    end
end
...
function stop_distribution(M::DomainSSP, state::State, s::Int)
    ...
    for (s′, state′) in enumerate(M.S)
        ...
        if state.pos != state′.pos || state.occ != state′.occ
            ...
            if state.tra == :empty
                if state′.tra == :empty || state′.tra == :low
                    p *= 0.5
                else
                    continue
                end
            elseif state.tra == :low
                if state′.tra == :empty
                    p *= 0.25
                elseif state′.tra == :low
                    p *= 0.5
                elseif state′.tra == :medium
                    p *= 0.25
                else
                    continue
                end
            elseif state.tra == :medium
                ...
                if state′.tra == :high || state′.tra == :medium
                    p *= 0.5
                else
                    continue
                end
            ...
            if (state′.tra == :empty && !state′.row) || (state.row && !state′.row)
                continue
            end
            if !state.row
