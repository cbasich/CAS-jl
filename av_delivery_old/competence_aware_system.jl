...
function human_cost(sh, state::CASstate, action::CASaction)
    if sh[3] == 1
        return [2*state.state.w.active_avs 2.0 1.0 0.0][action.l + 1]
    else
        if sh[2] == 1
            return [10.0 4.0 4.0 0.0][action.l + 1]
        else
            return [10.0 3.0 2.0 0.0][action.l + 1]
        end
    end
    ...
    for (s, state) in enumerate(S)
        ...
        for (a, action) in enumerate(A)
            ...
            if (t == [(base_s, 1.0)]  || action.l > κ[base_s][base_a] ||
                (state.sh[3] == 2 && state.sh[2] == 1 && κ[base_s][base_a] ∈ [0, 2]) ||
                (state.sh[3] == 2 && state.sh[2] == 2 && κ[base_s][base_a] == 0))
            ...
            if action.l == 0
                if typeof(state.state) == EdgeState
                    ...
                    for (sh′, p) in th
                        push!(T[s][a], (C.SIndex[CASstate(sh′, state′, '∅')], p))
                    end
            ...
            elseif action.l == 2
                ...
                for (sh′, p) in th
                    if typeof(state.state) == EdgeState
                        state′ = CASstate(sh′, EdgeState(state.state.u, state.state.v,
                                 state.state.θ, false, state.state.l, state.state.r, state.state.w), '∅')
                        push!(T[s][a], (C.SIndex[state′], p * p_override))
                        # T[s][a] = [(C.SIndex[state′], 1.0)]
                    else
                        # sp = (t[argmax([x[2] for x in t])][1]-1) * 4 + 4
                        state′ = 𝒟.S[t[argmax([x[2] for x in t])][1]]
                        push!(T[s][a], (C.SIndex[CASstate(sh′, state′, '∅')], p * p_override))
                        # T[s][a] = [((t[argmax([x[2] for x in t])][1]-1) * 4 + 4 , 1.0)]
                    end
                    # state′ = CASstate(sh′, EdgeState(state.state.u, state.state.v,
                    #          state.state.θ, false, state.state.l), '∅')
                    # push!(T[s][a], (C.SIndex[state′], p_override*p))
                    for (sp, p2) in t
                        push!(T[s][a], (C.SIndex[CASstate(sh′, 𝒟.S[sp], '∅')], p_null * p2))
