...
function update_cas_autonomy_profile!(C, ℒ)
    κ = C.𝒮.A.κ
    for (s, state) in enumerate(C.𝒮.D.S)
        ...
        for (a, action) in enumerate(C.𝒮.D.A)
            ...
            elseif L[i] == 0
                if C.𝒮.F.λ[s][a][0]['⊕'] > 0.05 || competence(state, action) == 2
                    C.potential[s][a][L[i] + 1] = 0.0
                    continue
                end
            elseif L[i] == κ[s][a]
...
function update_cas_feedback_profile!(C)
    ...
    for (a, action) in enumerate(A)
        for l=0:1
            ...
            try
                X_n, Y_n = split_data(D["node"][string(action.value)][l])
                M_n = build_forest(Y_n, X_n, -1, 11, 0.7, -1)
            catch
            ...
            if action.value ∈ ['↑', '⤉']
                try
                    X_e, Y_e = split_data(D["edge"][string(action.value)][l])
                    M_e = build_forest(Y_e, X_e, -1, 11, 0.7, -1)
                catch
            ...
            for (s, state) in enumerate(S)
                ...
                if typeof(state) == NodeState
                    if failed_to_build_node
                        ...
                        pred = []
                        try
                            pred = apply_forest_proba(M_n, f, [0,1])
                        catch
                            pred = [0.5 0.5]
                        end
                        if pred[1] == 1.0
                    ...
                    if failed_to_build_edge
                        ...
                        pred = []
                        try
                            pred = apply_forest_proba(M_e, f, [0,1])
                        catch
                            pred = [0.5 0.5]
                        end
                        if pred[1] == 1.0
...
function save_data(D)
    for k in keys(D["edge"])
        for l=0:1
            record_data(D["edge"][k][l], joinpath(abspath(@__DIR__), "data", "level_$l", "edge_$k.csv"), false)
        end
    end
    for k in keys(D["node"])
        for l=0:1
            record_data(D["node"][k][l], joinpath(abspath(@__DIR__), "data", "level_$l", "node_$k.csv"), false)
        end
    end
end
...
function CASSP(𝒮::CAS,
    ...
    return CASSP(𝒮, S, A, T, C, s₀, G, SIndex, AIndex, potential, blocked)
end
...
function generate_transitions!(𝒟, 𝒜, ℱ, C,
    ...
    T = C.T
    κ, λ = 𝒜.κ, ℱ.λ
    S_len, A_len = length(S), length(A)
    Threads.@threads for s = 1:S_len
        state = S[s]
        ...
        for a=1:len_A
            ...
            if (t == [(base_s, 1.0)]  || action.l > κ[base_s][base_a])
                ...
                if action.l == 2
                    push!(T[s][a], (C.SIndex[CASstate(dstate′, '⊕')], 1.0))
                else
                ...
                if typeof(state.state) == EdgeState
                    ...
                    elseif !state.state.o && action.action.value == '↑'
                        ...
                        for j=1:length(temp)
                            ...
                            p = (temp[j][2]/mass)*p_approval
                            if p > 0.0
                                push!(T[s][a], (C.SIndex[state′], p))
                            end
                        end
                        dstate′ = EdgeState(state.state.u, state.state.v,
                                state.state.θ, state.state.o, state.state.l,
                    ...
                    for j = 1:length(t)
                        p = t[j][2] * p_approval
                        if p > 0.0
                            push!(T[s][a], (C.SIndex[CASstate(𝒟.S[t[j][1]], '∅')], p))
                        end
                    end
                    dstate′ = NodeState(state.state.id, state.state.p,
                        state.state.o, state.state.v, state.state.θ, w)
            ...
            elseif action.l == 1
                p_approval = λ[base_s][base_a][1]['⊕']
                p_disapproval = 1.0 - p_approval #λ[base_s][base_a][1]['⊖']
                if typeof(state.state) == NodeState
                    dstate′ = NodeState(state.state.id, state.state.p,
                        state.state.o, state.state.v, state.state.θ, w)
                else
...
function generate_successor(C::CASSP,
                            ...
                            a::Int,
                            σ::Char)
    state = C.S[sample(first.(C.T[s][a]), aweights(last.(C.T[s][a])))]
    if terminal(C, state)
        return state
    end
    i = 1
